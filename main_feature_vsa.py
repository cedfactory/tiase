...
def cac40():
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".csv"):
            ...
            df = fprocessfeature.process_features(df, ['sort_df_by_corr'])


            df = fprocessfeature.process_features(df, ['data_labeling'])

            df = fdataprep.process_technical_indicators(df, ['outliers_cut_stdcutoff'])


            df = fprocessfeature.process_features(df, ['stationary_transform'])
